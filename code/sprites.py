...
class Icon(pygame.sprite.Sprite):
    ...
    def find_direction(self):
        if self.path:
            if self.rect.centerx == self.path[0][0]: # can move on the vertical axis
                self.direction = vector(0, 1 if self.path[0][1] > self.rect.centery else -1 )
            else: # can move on the horizontal axis
                self.direction = vector(1 if self.path[0][0] > self.rect.centerx else -1, 0)
        else:
            self.direction = vector()

    def point_collision(self): # vertical movement
        # When the icon exceeds the first point on the path, sets the player back to the center of the current point, deletes the current point, then finally checks for the path again
